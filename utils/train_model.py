...
def train_model(model,optimizer,scheduler,device,training_generator,validation_generator,log,logger,opt):
    ...
    if opt.resume:
        checkpoint = opt.save_path + '/stepall' + opt.checkpoint_path + '/' + opt.checkpoint
    ...
    for epoch in np.arange(start_epoch, end_epoch):
        ...
        if opt.rank==0:
            if epoch%opt.save_epoch == opt.save_epoch - 1:
                save_checkpoint({
                    ...
                    'optimizer': optimizer.state_dict()}, os.path.join(path_save, 'stepall', 'checkpoint_'+str(epoch+1)))
        ...
        if mean_val_loss < (best_val_loss - 1e-4):
            if opt.rank==0:
                ...
                save_checkpoint({
                    'epoch': epoch + 1,
                    'model': model.state_dict(),
                    'optimizer': optimizer.state_dict()}, os.path.join(path_save, 'stepall', 'checkpoint_best_loss'))
        ...
        if opt.rank==0:
            ...
            with open(os.path.join(path_save, 'stepall','learning_results.json'), 'w') as handle:
