...
from utils.helper import distance, BFS, equal_to_zero, centroid
...
class Postprocess(Module):
    ...
    def postpro_implement_v2(self, pred_vol):
        ...
        for group in group_list:
            points, mass_list, points_bol = [], [], []
            for point in group:
                points.append(xyz_rec[point, :])
                points_bol.append(xyz_bool[point, :])
                mass_list.append(conf_rec[point])
            cen_xyz_rec.append(list(centroid(points, mass_list)))
        ...
        for p in pro_valid_points:
            cen_xyz_rec_f.append(cen_xyz_rec[p])
            cen_conf_rec_f.append(cen_conf_rec[p])
            cen_xyz_bol_f.append(cen_xyz_bol[p])

        cen_xyz_rec_f = torch.tensor(cen_xyz_rec_f)
        cen_conf_rec_f = torch.tensor(cen_conf_rec_f)
        cen_xyz_bol_f = torch.tensor(cen_xyz_bol_f)

        return cen_xyz_rec_f, cen_conf_rec_f, cen_xyz_bol_f
    ...
    def forward(self, pred_vol):
        if self.postpro:
            xyz_rec, conf_rec, xyz_bool = self.postpro_implement_v2(pred_vol)
        else:
            xyz_rec, conf_rec, xyz_bool = self.no_postpro_implement(pred_vol)
