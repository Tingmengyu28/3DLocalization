...
def normal(f, x0, threshold = 100, tol = 1e-2):
    x = x0.detach()
...
def analyze_broyden(res_info, err=None, judge=True, name='forward', training=True, save_err=True):
    ...
    if torch.isnan(res_est).any():
        msg = "WARNING: nan found in Broyden's {name} result. Diff: {diff}"
        print(msg)
    ...
    if nstep == 0 and (diff != diff or diff > eps):
        msg = "WARNING: Bad Broyden's method {name}. Why?? Diff: {diff}. STOP."
        print(msg)
    ...
    if prot_break and np.random.uniform(0,1) < 0.05:
        msg = "WARNING: Hit Protective Break in {name}. Diff: {diff}. Total Iter: {len(trace)}"
        print(msg)
